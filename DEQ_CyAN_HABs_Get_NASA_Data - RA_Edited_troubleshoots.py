...
def program(year):
    ...
    # print(tif_file_path)
    #tif_file_path = "\\deqhq1\wq-share\Harmful Algal Blooms Coordination Team\HAB_Shiny_app\data\\" + str(year) + "\*.tif"
    tif_file_path = "\\deqhq1\wq-share\Harmful Algal Blooms Coordination Team\HAB_Shiny_app\data\\2017" +  "\*.tif"
    list_of_files = glob.glob(tif_file_path)  # Change directory as needed
    for latest_file in list_of_files: #NEW here rather than just looking at the latest file we will iterate through all of them
        ...
        for i in range(0, hab_days_length):
            ...
            dir_Shiny = "\\\\deqhq1\\wq-share\\Harmful Algal Blooms Coordination Team\\HAB_Shiny_app\\data\\Output\\" + str(year) #NEW this you will either need to manually add the folder name which is just the year or use and OS.mkdir command before 
            thetable = os.path.join(dir_Shiny, 'HAB_resolvablelakes_' + year + '-' + hab_day_start + '.xlsx') #NEWthis is where it is writing to so we now make it unique according to year and day
